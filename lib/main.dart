...
class BilliBilliLogo extends StatelessWidget {
  const BilliBilliLogo({
    super.key,
    this.size = 42,
    this.radius = 10,
  });
...
class AppData {
  ...
  Future<String?> _prepareProfilePhoto(String sourcePath) async {
    ui.Codec? codec;
    ui.Image? source;
    ui.Image? cropped;
    ui.Picture? picture;
    try {
      ...
      // Profile photos are stored as a true square. This deliberately crops
      // the longer dimension instead of fitting the original image into a
      // square, so no white/empty letterbox strip can be introduced.
      final side = source.width < source.height ? source.width : source.height;
      final left = (source.width - side) / 2.0;
      final top = (source.height - side) / 2.0;
      ...
      canvas.drawImageRect(
        ...
        const ui.Rect.fromLTWH(0, 0, outputSize, outputSize),
        paint,
      );

      picture = recorder.endRecording();
      cropped = await picture.toImage(1024, 1024);
      final png = await cropped.toByteData(format: ui.ImageByteFormat.png);
      if (png == null) return null;

      final dir = await _mediaDirectory();
      final path = '${dir.path}/billi_profile_${DateTime.now().millisecondsSinceEpoch}.png';
  ...
  Future<String?> pickAndSaveProfilePhoto(ImageSource source) async {
    try {
      ...
      // Always center-crop the profile image to a square before saving.
      // This removes portrait/landscape letterbox strips and keeps the
      // displayed profile photo clean and consistent.
      final preparedPath = await _prepareProfilePhoto(picked.path);
      final savedPath = preparedPath ?? await copyMediaToAppFolder(picked.path);
            ...
            slivers: [
              SliverAppBar(
                ...
                title: const Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    BilliBilliLogo(size: 38, radius: 8),
                    SizedBox(width: 10),
                    Text(
                      'Billi Billi',
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                await AppData.instance.addComment(widget.post, text: comment);
                if (dialogContext.mounted) Navigator.pop(dialogContext);
                if (mounted) setState(() {});
              },
              child: const Text('Post'),
